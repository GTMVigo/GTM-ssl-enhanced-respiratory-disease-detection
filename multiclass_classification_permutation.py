...
# ============================================================================
# PERMUTATION TESTING CONSTANTS
# ============================================================================

N_PERMUTATIONS = 1000  # Number of permutations for CI calculation
CI_ALPHA = 0.05  # Significance level for confidence intervals
RANDOM_STATE = 42  # For reproducibility
N_SPLITS = 5  # Number of KFold cross-validation splits


# ============================================================================
# PERMUTATION TESTING HELPER FUNCTIONS
# ============================================================================

def compute_permutation_test(predictions: np.ndarray,
                             true_labels: np.ndarray,
                             audio_labels: list,
                             n_permutations: int = N_PERMUTATIONS,
                             metric: str = 'f1') -> Dict[str, Any]:
    '''
    Compute permutation test to obtain confidence intervals for model performance.
    
    Args:
        predictions (np.ndarray):  Model predictions on test set.
        true_labels (np.ndarray):  True labels for test set.
        audio_labels (list):       Audio label information.
        n_permutations (int):      Number of permutation iterations.
        metric (str):              Metric to compute ('f1', 'accuracy').
    
    Returns:
        Dict[str, Any]: Dictionary containing:
            - original_score: Score on original (non-permuted) labels
            - permutation_scores: Array of scores from permuted labels
    ...
    # Extract true labels from audio_labels if needed
    # if isinstance(true_labels, list) and len(true_labels) > 0:
    #     if isinstance(true_labels[0], (list, tuple)):
    #         y_true = np.array([label[0] for label in true_labels])
    #     else:
    ...
    predictions = predictions.astype(float).astype(int)
    # Compute original score
    if metric.lower() == 'f1':
        original_score = f1_score(y_true, predictions, average='binary', zero_division=0)
    elif metric.lower() == 'accuracy':
        original_score = accuracy_score(y_true, predictions)
    else:
        original_score = f1_score(y_true, predictions, average='binary', zero_division=0)
    ...
    for _ in range(n_permutations):
        ...
        # Compute metric on permuted labels
        if metric.lower() == 'f1':
            perm_score = f1_score(y_permuted, predictions, average='binary', zero_division=0)
        elif metric.lower() == 'accuracy':
            perm_score = accuracy_score(y_permuted, predictions)
        else:
            perm_score = f1_score(y_permuted, predictions, average='binary', zero_division=0)
    ...
    alpha = CI_ALPHA
    ci_lower = np.percentile(permutation_scores, alpha/2 * 100)
    ci_upper = np.percentile(permutation_scores, (1 - alpha/2) * 100)
    
    # Compute p-value (proportion of permuted scores >= original with continuity correction)
    p_value = (np.sum(permutation_scores >= original_score) + 1) / (n_permutations + 1)
    ...
    return {
        ...
        'mean_permuted': float(np.mean(permutation_scores)),
        'std_permuted': float(np.std(permutation_scores, ddof=1)),
        'p_value': float(p_value),
        'n_permutations': n_permutations
    }


def save_permutation_results(permutation_data: Dict[str, Any],
                             output_path: str,
                             task_moment: str = '',
                             which_feature: str = '') -> None:
    '''
    Save permutation test results to CSV file.
    
    Args:
        permutation_data (Dict[str, Any]): Permutation test results.
        output_path (str):                 Path to save results.
        task_moment (str):                 Task-moment identifier.
        which_feature (str):               Feature identifier.
    ...
    results_dict = {
        ...
        'original_score': permutation_data['original_score'],
        'ci_lower': permutation_data['ci_lower'],
        'ci_upper': permutation_data['ci_upper'],
        'mean_permuted': permutation_data['mean_permuted'],
        'std_permuted': permutation_data['std_permuted'],
        'p_value': permutation_data['p_value'],
        'n_permutations': permutation_data['n_permutations'],
        'score_gap': permutation_data['original_score'] - permutation_data['mean_permuted']
    }
...
def save_permutation_fold_results(permutation_results_list: List[Dict[str, Any]],
                                  model_name: str,
                                  which_feature: str,
                                  output_dir: str,
                                  logger: BasicLogger = None) -> str:
    '''
    Save per-fold permutation test results to CSV file.
    
    Args:
        permutation_results_list (List[Dict[str, Any]]): List of permutation results from each fold.
    ...
    results_list = []
    for fold_idx, perm_data in enumerate(permutation_results_list):
        results_dict = {
            'fold': fold_idx,
            'original_score': perm_data['original_score'],
            'ci_lower': perm_data['ci_lower'],
            'ci_upper': perm_data['ci_upper'],
            'mean_permuted': perm_data['mean_permuted'],
            'std_permuted': perm_data['std_permuted'],
            'p_value': perm_data['p_value'],
            'n_permutations': perm_data['n_permutations'],
            'score_gap': perm_data['original_score'] - perm_data['mean_permuted']
        }
    ...
    mean_results = {
        'fold': 'mean',
        'original_score': np.mean([r['original_score'] for r in results_list]),
        'ci_lower': np.mean([r['ci_lower'] for r in results_list]),
        'ci_upper': np.mean([r['ci_upper'] for r in results_list]),
        'mean_permuted': np.mean([r['mean_permuted'] for r in results_list]),
        'std_permuted': np.mean([r['std_permuted'] for r in results_list]),
        'p_value': np.mean([r['p_value'] for r in results_list]),
        'n_permutations': N_PERMUTATIONS,
        'score_gap': np.mean([r['score_gap'] for r in results_list])
    }
    ...
    std_results = {
        'fold': 'std',
        'original_score': np.std([r['original_score'] for r in results_list[:-1]], ddof=1),
        'ci_lower': np.std([r['ci_lower'] for r in results_list[:-1]], ddof=1),
        'ci_upper': np.std([r['ci_upper'] for r in results_list[:-1]], ddof=1),
        'mean_permuted': np.std([r['mean_permuted'] for r in results_list[:-1]], ddof=1),
        'std_permuted': np.std([r['std_permuted'] for r in results_list[:-1]], ddof=1),
        'p_value': np.std([r['p_value'] for r in results_list[:-1]], ddof=1),
        'n_permutations': N_PERMUTATIONS,
        'score_gap': np.std([r['score_gap'] for r in results_list[:-1]], ddof=1)
    }
    results_list.append(std_results)
    
    results_df = pd.DataFrame(results_list)
    output_path = os.path.join(output_dir, f"permutation_folds_{model_name}_{which_feature}.csv")
    results_df.to_csv(output_path, index=False)
    
    if logger:
        logger.info(f"✓ Per-fold permutation results saved: {output_path}")
    else:
        print(f"✓ Per-fold permutation results saved: {output_path}")
    
    return output_path


def save_permutation_cross_fold_summary(permutation_results_list: List[Dict[str, Any]],
                                        model_name: str,
                                        which_feature: str,
                                        output_dir: str,
                                        ...
                                        logger: BasicLogger = None) -> str:
    '''
    Save cross-fold permutation test summary to CSV file.
    ...
    original_scores = np.array([perm_data['original_score'] for perm_data in permutation_results_list])
    ci_lowers = np.array([perm_data['ci_lower'] for perm_data in permutation_results_list])
    ci_uppers = np.array([perm_data['ci_upper'] for perm_data in permutation_results_list])
    mean_permuteds = np.array([perm_data['mean_permuted'] for perm_data in permutation_results_list])
    std_permuteds = np.array([perm_data['std_permuted'] for perm_data in permutation_results_list])
    p_values = np.array([perm_data['p_value'] for perm_data in permutation_results_list])
    score_gaps = original_scores - mean_permuteds
    
    summary_dict = {
        'which_feature': which_feature,
        'n_folds': n_folds,
        'original_score_mean': float(np.mean(original_scores)),
        'original_score_std': float(np.std(original_scores, ddof=1)),
        'ci_lower_mean': float(np.mean(ci_lowers)),
        'ci_lower_std': float(np.std(ci_lowers, ddof=1)),
        'ci_upper_mean': float(np.mean(ci_uppers)),
        'ci_upper_std': float(np.std(ci_uppers, ddof=1)),
        'mean_permuted_mean': float(np.mean(mean_permuteds)),
        'mean_permuted_std': float(np.std(mean_permuteds, ddof=1)),
        'std_permuted_mean': float(np.mean(std_permuteds)),
        'std_permuted_std': float(np.std(std_permuteds, ddof=1)),
        'p_value_mean': float(np.mean(p_values)),
        'p_value_std': float(np.std(p_values, ddof=1)),
        'score_gap_mean': float(np.mean(score_gaps)),
        'score_gap_std': float(np.std(score_gaps, ddof=1)),
        'n_permutations': N_PERMUTATIONS
    }
    
    summary_df = pd.DataFrame([summary_dict])
    output_path = os.path.join(output_dir, f"permutation_summary_{model_name}_{which_feature}.csv")
    summary_df.to_csv(output_path, index=False)
    
    if logger:
        logger.info(f"✓ Cross-fold permutation summary saved: {output_path}")
    else:
        print(f"✓ Cross-fold permutation summary saved: {output_path}")
...
def load_and_test_model(config: dict,
                        ...
                        run_permutation_test: bool = False) -> tuple[Dict[str, float], 
                                                                      pd.DataFrame,
                                                                      Dict[str, Any]]:   
    '''
    ...
    Args:
        ...
        run_permutation_test (bool):     Whether to run permutation test on predictions.
    ...
    Returns:
        tuple[Dict[str, float], pd.DataFrame, Dict[str, Any]]:
            ...
            - permutation_results (Dict[str, Any]): Dictionary containing permutation test results.
    '''
    ...
    # Test the model
    metrics_dict, detailed_pd, _, permutation_results = test_model(model_builder = model_builder, 
    ...
    return metrics_dict, detailed_pd, permutation_results


def evaluate_model(config: Dict[str, Any], 
                   run_permutation_test: bool = True,
                   use_kfold: bool = True) -> None:
    '''
    Evaluate the model based on the configuration, with optional KFold cross-validation and permutation testing.

    Args:
        config (Dict[str, Any]):  Configuration dictionary.
        run_permutation_test (bool): Whether to run permutation tests for CI estimation.
        use_kfold (bool): Whether to use KFold cross-validation. If False, uses full dataset.
    ...
    # Prepare output directory for permutation results
    if run_permutation_test:
        perm_output_dir = os.path.join(eval_conf['save_metrics_path'], 'permutation_results')
        os.makedirs(perm_output_dir, exist_ok=True)
        logger.info(f"Permutation test results will be saved to: {perm_output_dir}")
    ...
    for which_feature in eval_conf['which_features']:
        ...
        # Evaluate all IDs for cross-validation
        features_id, _ = get_data_to_split(labels = labels_dict)
        features_id = features_id
        
        # Initialize model builder (will be reused for each fold)
        model_builder = ModelBuilder(name = eval_conf['model'],
                                     save_name = eval_conf['model_name'],
                                     path_to_model = eval_conf['model_path'],
                                     app_logger = logger)
        model_builder.build_model()
        model_builder.load_model_from_a_serialized_object()
        
        if use_kfold:
            # KFold cross-validation with permutation testing
            logger.info(f"\n{'='*70}")
            logger.info(f"KFold Cross-Validation ({N_SPLITS} folds) with Permutation Testing")
            logger.info(f"Feature: {which_feature}")
            logger.info(f"{'='*70}")
            
            kfold = KFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_STATE)
            fold_permutation_results = []
            
            for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(features_id)):
                logger.info(f"\n--- Fold {fold_idx + 1}/{N_SPLITS} ---")
                
                # Test the model on this fold
                fold_metrics, fold_detailed_pd, _, fold_perm_results = test_model(
                ...
                fold_permutation_results.append(fold_perm_results)
                
                # Log fold results
                if run_permutation_test and fold_perm_results:
                    logger.info(f"Fold {fold_idx} - Original Score: {fold_perm_results['original_score']:.4f}, "
                               f"CI: [{fold_perm_results['ci_lower']:.4f}, {fold_perm_results['ci_upper']:.4f}]")
            
            # Save per-fold results and cross-fold summary
            if run_permutation_test and fold_permutation_results:
                save_permutation_fold_results(
                    permutation_results_list = fold_permutation_results,
                ...
                save_permutation_cross_fold_summary(
                    permutation_results_list = fold_permutation_results,
                    model_name = eval_conf['model_name'],
                    which_feature = which_feature,
                    output_dir = perm_output_dir,
                ...
                # Compute and log cross-fold summary
                original_scores = np.array([r['original_score'] for r in fold_permutation_results])
                logger.info(f"\n{'='*70}")
                logger.info(f"Cross-Fold Summary ({which_feature}):")
                logger.info(f"{'='*70}")
                logger.info(f"  Original Score: {np.mean(original_scores):.4f} ± {np.std(original_scores, ddof=1):.4f}")
                logger.info(f"  Mean CI Lower: {np.mean([r['ci_lower'] for r in fold_permutation_results]):.4f}")
                logger.info(f"  Mean CI Upper: {np.mean([r['ci_upper'] for r in fold_permutation_results]):.4f}")
                logger.info(f"  Mean P-value: {np.mean([r['p_value'] for r in fold_permutation_results]):.4f}")
                
                print(f"\n✓ KFold permutation testing complete for {which_feature}")
                print(f"  Mean Score: {np.mean(original_scores):.4f} ± {np.std(original_scores, ddof=1):.4f}")
        
        else:
            # Full dataset evaluation (no cross-validation)
            metrics, detailed_pd, permutation_results = load_and_test_model(
                config = eval_conf,
                labels_dict = labels_dict,
                feature_type = feature_types[0],
                which_feature = which_feature,
            ...
            if run_permutation_test and permutation_results:
                perm_csv_path = os.path.join(perm_output_dir,
                                            f"permutation_ci_{eval_conf['model_name']}_{which_feature}.csv")
                save_permutation_results(
                    permutation_data = permutation_results,
                    output_path = perm_csv_path,
                    task_moment = '',
                    which_feature = which_feature
                )
                
                # Log permutation results
                logger.info(f"\nPermutation Test Results ({which_feature}):")
                logger.info(f"  Original F1 Score: {permutation_results['original_score']:.4f}")
                logger.info(f"  95% CI: [{permutation_results['ci_lower']:.4f}, {permutation_results['ci_upper']:.4f}]")
                logger.info(f"  Mean Permuted Score: {permutation_results['mean_permuted']:.4f} ± {permutation_results['std_permuted']:.4f}")
                logger.info(f"  P-value: {permutation_results['p_value']:.4f}")
                logger.info(f"  Score Gap: {permutation_results['score_gap']:+.4f}")
                logger.info(f"  Permutation count: {permutation_results['n_permutations']}")
                
                print(f"\n✓ Permutation test for {which_feature} complete")
                print(f"  Original F1: {permutation_results['original_score']:.4f}")
                print(f"  95% CI: [{permutation_results['ci_lower']:.4f}, {permutation_results['ci_upper']:.4f}]")
...
def test_model(model_builder: ModelBuilder, 
    ...
    Args:
        ...
        metrics_dict (Dict[str, float]): Dictionary to store evaluation metrics.
        run_permutation_test (bool):     Whether to run permutation test.

    Returns:
        tuple[Dict[str, float], pd.DataFrame, Dict, Dict[str, Any]]:
            - metrics_dict (Dict[str, float]):       Updated metrics dictionary.
            - detailed_pd (pd.DataFrame):            DataFrame containing detailed predictions and correctness checks.
    ...
    test_features, test_labels, audio_labels = get_features_and_labels(labels_dict = labels_dict,
    ...
    if run_permutation_test:
        try:
            ...
            print(f"Warning: Could not run permutation test: {e}")
...
def extract_train_test_data(config: Dict[str, Any], run_permutation_test: bool = True) -> None:
    '''
        ...
        for which_feature in model_conf['which_features']:
            ...
            for model in model_conf['model_list']:
                ...
                for fold, (train_index, test_index) in enumerate(train_test_split):
                    ...
                    # Test the model
                    metrics_dict, detailed_pd, threshold_results, permutation_results = test_model(
                        model_builder=model_builder,
                    ...
                    if permutation_results:
                        fold_permutation_results.append(permutation_results)
                        perm_dir = os.path.join(
                            os.path.dirname(model_conf['save_metrics_path']),
                            'permutation_results'
                        )
                        os.makedirs(perm_dir, exist_ok=True)

                        perm_csv_path = os.path.join(
                            perm_dir,
                            f"permutation_ci_{model_name}_{which_feature}_fold{fold}.csv"
                        )

                        task_moment = os.path.splitext(
                            os.path.basename(model_conf['save_metrics_path'])
                        )[0]
                ...
                if run_permutation_test and fold_permutation_results:
                    ...
                    # Generate a uniform base model name identifier without specific fold suffix
                    base_model_name = f"{model}_{feature_type_string.rstrip('_')}"
                    ...
                    save_permutation_cross_fold_summary(
                        ...
                        which_feature = which_feature,
                        output_dir = perm_dir,
                        n_folds = len(fold_permutation_results),
                        logger = logger
                    )              

def main(config: Dict[str, Any], run_permutation_test: bool = True) -> None:
    '''
    ...
    '''
    print(f"\n{'='*80}")
    print("MULTICLASS CLASSIFICATION WITH PERMUTATION TESTING")
    print(f"{'='*80}")
    if run_permutation_test:
        print(f"Permutation test enabled with {N_PERMUTATIONS} permutations (95% CI)")
    else:
        print("Permutation test disabled")
    print(f"{'='*80}\n")
    
    if config['run']['extract_train_test']:
        extract_train_test_data(config=config, run_permutation_test=run_permutation_test)
    elif config['run']['evaluate_model']:
        evaluate_model(config = config, run_permutation_test = run_permutation_test)
...
if __name__ == '__main__':
    ...
    # Run with permutation testing enabled (1000 permutations for CI calculation)
    main(config = configuration, run_permutation_test = True)